    ...
    function getStations() {
        ...
        return (
            <Grid container>
                ...
                <Grid size={{ xs: 12, md: 2 }}>{'LCD = ' + station1}</Grid>
                <Grid size={{ xs: 12, md: 2 }}>{buildLCString(station1LC)}</Grid>
                <Grid size={{ xs: 12, md: 3 }}>{crString(lcdToCRInfo(station1), names)}</Grid>
                <Grid size={{ xs: 12, md: 3 }}>{'Station ' + getNumStations(station1)}</Grid>
                <Grid size={{ xs: 12, md: 2 }}>{ DIRECTIONS[getDirectionColorIndex(station1CR.tzolkinName)] + ' ' + COLORS[getDirectionColorIndex(station1CR.tzolkinName)] }</Grid>
                <Grid size={{ xs: 12, md: 12}}><div><br></br></div></Grid>
                <Grid size={{ xs: 12, md: 2 }}>{'LCD = ' + station2}</Grid>
                <Grid size={{ xs: 12, md: 2 }}>{buildLCString(station2LC)}</Grid>
                <Grid size={{ xs: 12, md: 3 }}>{crString(lcdToCRInfo(station2), names)}</Grid>
                <Grid size={{ xs: 12, md: 3 }}>{'Station ' + getNumStations(station2)}</Grid>
                <Grid size={{ xs: 12, md: 2 }}>{ DIRECTIONS[getDirectionColorIndex(station2CR.tzolkinName)] + ' ' + COLORS[getDirectionColorIndex(station2CR.tzolkinName)] }</Grid>
        ...
        return(
            <Grid container>
                <Grid size={{ md: 12 }}>{getStations()}</Grid>
                <Grid size={{ md: 12}}>
