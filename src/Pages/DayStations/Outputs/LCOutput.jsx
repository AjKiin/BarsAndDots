    ...
    if (getPlaceIn819Cycle(lcd) === 1) {
        ...
        return (
            <Grid container>
                ...
                <Grid size={{ xs: 12, md: 12 }}>{buildLCString(lc) + ' ' + crString(cr, names) + ' is an 819 day station'}</Grid>
                <Grid size={{ xs: false, md: 3 }}></Grid>
                <Grid size={{ xs: 12, md: 2 }}>{'Station ' + getNumStations(lcd)}</Grid>
                <Grid size={{ xs: 12, md: 2 }}>{'Color: ' + COLORS[getDirectionColorIndex(cr.tzolkinName)]}</Grid>
                <Grid size={{ xs: 12, md: 2 }}>{'Direction: ' + DIRECTIONS[getDirectionColorIndex(cr.tzolkinName)]}</Grid>
                <Grid size={{ xs: false, md: 3 }}></Grid>
        ...
        return (
            <Grid container>
                ...
                <Grid size={{ xs: 12, md: 12 }}>{buildLCString(lc) + ' ' + crString(lcdToCRInfo(lcd), names) + ' is NOT an 819 day station'}</Grid>
                
                <Grid size={{ xs: 12, md: 12 }}><div><br></br></div></Grid>
                <Grid size={{ xs: 12, md: 12 }}>The nearest 819 Day Stations are {daysBackward} day(s) prior at:</Grid>
                <Grid size={{ xs: 12, md: 12 }}><div><br></br></div></Grid>

                <Grid size={{ xs: 12, md: 2 }}></Grid>
                <Grid size={{ xs: 6, md: 2 }}>{buildLCString(lcPrior)}</Grid>
                <Grid size={{ xs: 6, md: 2 }}>{crString(crPrior, names)}</Grid>
                <Grid size={{ xs: 6, md: 2 }}>Station {getNumStations(priorStation)}</Grid>
                <Grid size={{ xs: 6, md: 2 }}>{ DIRECTIONS[getDirectionColorIndex(crPrior.tzolkinName)] + ' ' + COLORS[getDirectionColorIndex(crPrior.tzolkinName)] }</Grid>
                
                <Grid size={{ xs: 12, md: 12 }}><div><br></br></div></Grid>
                <Grid size={{ xs: 12, md: 12 }}>And {daysForward} day(s) forward at:</Grid>
                <Grid size={{ xs: 12, md: 12 }}><div><br></br></div></Grid>

                <Grid size={{ xs: 12, md: 2 }}></Grid>
                <Grid size={{ xs: 6, md: 2 }}>{buildLCString(lcForward)}</Grid>
                <Grid size={{ xs: 6, md: 2 }}>{crString(crForward, names)}</Grid>
                <Grid size={{ xs: 6, md: 2 }}>Station {getNumStations(forwardStation)}</Grid>
                <Grid size={{ xs: 6, md: 2 }}>{ DIRECTIONS[getDirectionColorIndex(crForward.tzolkinName)] + ' ' + COLORS[getDirectionColorIndex(crForward.tzolkinName)] }</Grid>
