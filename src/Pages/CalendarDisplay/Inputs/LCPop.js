...
export default function LCPop (props) {
    ...
    function handlePiktunChange (event) {
        ...
        if (event.target.value !== 9) {
            ...
            calcLCD(findNearestPop(getLCDFromLC( PIKTUN_VALUES[event.target.value], baktuns, katuns, tuns, winals, kins)));
        } 
        ...
        // Now handle the short piktun, and automatically choose baktun = 0 when piktun = 19 (piktun index = 9)
            ...
            setPiktuns(event.target.value);
            setBaktuns(0);
            calcLCD(findNearestPop(getLCDFromLC( PIKTUN_VALUES[event.target.value], 0, katuns, tuns, winals, kins)));
        }
    }
    ...
    function handleBaktunChange (event) {
        if (event.target.value >= -260 && event.target.value <= 260) {
            ...
            calcLCD(findNearestPop(getLCDFromLC( PIKTUN_VALUES[piktuns], event.target.value, katuns, tuns, winals, kins)));
        }
    }
    ...
    function handleKatunChange (event) {
        if (event.target.value >= 0 && event.target.value <= 19) {
            ...
            calcLCD(findNearestPop(getLCDFromLC( PIKTUN_VALUES[piktuns], baktuns, event.target.value, tuns, winals, kins)));
        }
    }
    ...
    function handleTunChange (event) {
        if (event.target.value >= 0 && event.target.value <= 19) {
            ...
            calcLCD(findNearestPop(getLCDFromLC( PIKTUN_VALUES[piktuns], baktuns, katuns, event.target.value, winals, kins)));
        }
    }
    ...
    function handleWinalChange (event) {
        if (event.target.value >= 0 && event.target.value <= 17) {
            ...
            calcLCD(findNearestPop(getLCDFromLC( PIKTUN_VALUES[piktuns], baktuns, katuns, tuns, event.target.value, kins)));
        }
    }
    ...
    function handleKinChange (event) {
        if (event.target.value >= 0 && event.target.value <= 19) {
            ...
            calcLCD(findNearestPop(getLCDFromLC( PIKTUN_VALUES[piktuns], baktuns, katuns, tuns, winals, event.target.value)));
        }
    }

    // This finds the nearest 1 Pop date from the one given
    function findNearestPop(lcd) {
    ...
    return(
        ...
        <Grid container justifyContent="center">
            ...
            <Grid item md={6} sx={textboxStyleRight}></Grid>

            <Grid item xs={4} md={6} sx={textboxStyleLeft}>
            ...
            <Grid item xs={4} md={6} sx={textboxStyleRight}>
                <FormControl sx={{ m: 1, minWidth: 100 }}>
            ...
            <Grid item md={6} sx={textboxStyleLeft}></Grid>
