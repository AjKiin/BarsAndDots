...
export default function DatePop(props) {
    ...
    function handleTypeChange (event) {
        ...
        calcLCD(findNearestPop(getLCDFromJDN(changeToJDN( day, month, year, event.target.value), correlation)));
    }

    function handleYearChange (event) {
        if (event.target.value >= -3113 && event.target.value <= 4772) {
            setYear(event.target.value);
            calcLCD(findNearestPop(getLCDFromJDN(changeToJDN( day, month, event.target.value, type), correlation)));
        }
    }
    ...
    function handleMonthChange (event) {
        if (event.target.value >= 1 && event.target.value <= 12) {
            ...
            calcLCD(findNearestPop(getLCDFromJDN(changeToJDN( day, event.target.value, year, type), correlation)));
        }
    }
    ...
    function handleDayChange (event) {
        if (event.target.value >= 1 && event.target.value <= 31) {
            ...
            calcLCD(findNearestPop(getLCDFromJDN(changeToJDN( event.target.value, month, year, type), correlation)));
        }
    }
    ...
    return(
        <Grid container sx={{ justifyContent: "center" }}>
            ...
            <FormControl sx={{ m: 1, minWidth: 150 }}>
                <InputLabel id="GJ" htmlFor="gj-select">Select Calendar</InputLabel>
                <Select
                    inputProps={{ id: 'gj-select' }}
                    value={type}
            ...
            <Grid size={{ xs: 4, md: 12 }} sx={textboxStyle}>
                <TextField
                    id="year"
                    label="Year"
                    value={year}
